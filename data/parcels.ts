...
export const getParcelsInArea = (boundingBox: number[]): Parcel[] => {
    ...
    for (let i in parcels) {
      if (
        parcels[i].lat <= maxLat &&
        parcels[i].lat >= minLat &&
        parcels[i].lon <= maxLng &&
        parcels[i].lon >= minLng
      )
        parcelsInArea.push(parcels[i]);
    }
    
  
    return parcelsInArea;
  };